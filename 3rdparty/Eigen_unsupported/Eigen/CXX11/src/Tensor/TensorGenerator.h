 ...
 *
 * The TensorGeneratorOp is an expression class representing the application of a generator
 * expression to a tensor. A generator expression is a functor that takes a coordinate array
 * of the length of the tensor's rank and returns the value to assign to the corresponding
 * element of the tensor.
 *
 * The following example shows a generator for a rank-2 tensor, which assigns to each element
 * the value corresponding to the sum of 10 times its first coordinates and the second coordinate.
 *
 * @code
 *   struct Generator2D {
 *   Generator2D() { }
 *   float operator()(const array<Eigen::DenseIndex, 2>& coordinates) const {
 *     return 10 * coordinates[1] + coordinates[0];
 *   }
 *   };
 * @endcode
 *
 * The generator can be applied to a tensor as follows:
 * @code
