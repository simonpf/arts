...
/** \brief Coefficient wise application of unary operation.
 *
 * Expression class represnting the element-wise application of a unary expression
 * to a tensor.
 *
 * The following example applies the tangen hyperbolicus function to all elements
 * in a tensor:
 *
 * @code
 * result = tensor.unaryExpr(&tanhf);
 * @endcode
 */
template<typename UnaryOp, typename XprType>
