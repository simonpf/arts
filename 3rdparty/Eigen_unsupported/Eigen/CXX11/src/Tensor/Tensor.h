...
namespace detail {
    ...
    template <int M, int N, int Layout> struct IndexHelper;

    using Eigen::Tensor;
    using Eigen::TensorMap;

    template <int M, int N>
        struct IndexHelper<M, N, RowMajor> {
        template <typename Index>
        static Index get_offset(const DSizes<Index, M>& dimensions,
                                const DSizes<Index, N>& indices) {

            DSizes<Index, M> indices_filled{};
            for (int i = 0; i < N; ++i) {
                indices_filled[i] = indices[i];
            }
            return dimensions.IndexOfRowMajor(indices_filled);
        }

        template <typename Index>
        static DSizes<Index, M - N> get_remaining_size(const DSizes<Index, M>& dimensions) {
            DSizes<Index, M - N> sizes;
            for (int i = 0; i < M - N; ++i) {
                sizes[i] = dimensions[N + i];
            }
            return sizes;
        }
    };
    ...
    template <int M, int N>
    struct IndexHelper<M, N, ColMajor> {
      template <typename Index>
      static Index get_offset(const DSizes<Index, M>& dimensions,
                              const DSizes<Index, N>& indices) {
        DSizes<Index, M> indices_filled{};
        for (int i = 0; i < N; ++i) {
            indices_filled[M - N + i] = indices[i];
        }
        return dimensions.IndexOfColMajor(indices_filled);
      }

      template <typename Index>
      static DSizes<Index, M - N> get_remaining_size(
          const DSizes<Index, M>& dimensions) {
        DSizes<Index, M - N> sizes;
        for (int i = 0; i < M - N; ++i) {
    ...
    struct tensor_index_return {
    using Scalar = typename Tensor::Scalar;
    static constexpr int NumIndices = Tensor::NumIndices;
    static constexpr int Options = Tensor::Options;
    using Helper = IndexHelper<NumIndices, N, Options>;
    using ReturnType =
        TensorMap<const Eigen::Tensor<Scalar, NumIndices - N, Options>>;
    static ReturnType return_value(const Scalar *data, const DSizes<Index, NumIndices> &dimensions, const DSizes<Index, N> &indices) {
        DSizes<Index, NumIndices - N> sizes = Helper::get_remaining_size(dimensions);
        return ReturnType(data + Helper::get_offset(dimensions, indices), sizes);
    }
    };

    template <typename Scalar, int Options, int N>
    struct tensor_index_return<Tensor<Scalar, N + 2, Options>, N> {
    using MatrixType = Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic, Options>;
    using ReturnType = Map<const MatrixType>;
    using Helper = IndexHelper<N + 2, N, Options>;
    static ReturnType return_value(const Scalar *data, const DSizes<Index, N + 2> &dimensions, const DSizes<Index, N> &indices) {
        DSizes<Index, 2> sizes = Helper::get_remaining_size(dimensions);
        return ReturnType(data + Helper::get_offset(dimensions, indices), sizes[0], sizes[1]);
    }
    };

    template <typename Scalar, int Options, int N>
    struct tensor_index_return<Tensor<Scalar, N + 1, Options>, N> {
    using VectorType = Matrix<Scalar, 1, Eigen::Dynamic>;
    using ReturnType = Map<const VectorType>;
    using Helper = IndexHelper<N + 1, N, Options>;
    static ReturnType return_value(const Scalar *data, const DSizes<Index, N + 1> &dimensions, const DSizes<Index, N> &indices) {
        DSizes<Index, 1> sizes = Helper::get_remaining_size(dimensions);
        return ReturnType(data + Helper::get_offset(dimensions, indices), sizes[0]);
    }
    };
...
class Tensor : public TensorBase<Tensor<Scalar_, NumIndices_, Options_, IndexType_> >
{
  ...
  protected:
    ...
    template<long unsigned int M> struct isOfNormalIndex<const array<Index, M>&>{
  ...
  public:
    ...
    template<typename... IndexTypes, typename std::enable_if<sizeof ... (IndexTypes) + 2 == NumIndices>::type = 0>
...
public:
    ...
    template <long unsigned int M,
              EIGEN_SFINAE_ENABLE_IF(M < NumIndices)>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE  auto operator()(const array<Index, M>& indices) const
        -> typename detail::tensor_index_return<Tensor, M>::ReturnType
    {
        return detail::tensor_index_return<Tensor, M>::return_value(data(), dimensions(), DSizes<Index, M>(indices));
    }
