...
*/
void xml_read_from_stream(istream& is_xml,
                          ArrayOfScatteringParticle& array,
                          bifstream* pbifs,
                          const Verbosity& verbosity) {
    ArtsXMLTag tag(verbosity);
    Index nelem;
    ...
    tag.read_from_stream(is_xml);
    tag.check_name("Array");
    tag.check_attribute("type", "ArrayOfScatteringParticle");
    ...
    try {
        for (n = 0; n < nelem; n++)
            xml_read_from_stream(is_xml, array[n], pbifs, verbosity);
    } catch (const std::runtime_error& e) {
        ostringstream os;
        os << "Error reading ArrayOfScatteringParticle"
           << "\n Element: " << n << "\n"
           << e.what();
        throw runtime_error(os.str());
    }
...
//! Writes ArrayOfScatteringParticle to XML output stream.
/*!
  \param os_xml   XML Output stream
  \param array    The array of scattering particle objects to write.
  \param pbofs    Pointer to binary file stream. NULL for ASCII output.
  \param name     Optional name attribute
*/
void xml_write_to_stream(ostream& os_xml,
    ...
    open_tag.add_attribute("type", "ArrayOfScatteringParticle");
    open_tag.add_attribute("nelem", array.nelem());

    open_tag.write_to_stream(os_xml);
    os_xml << '\n';

    for (Index n = 0; n < array.nelem(); n++)
        xml_write_to_stream(os_xml, array[n], pbofs, "", verbosity);
